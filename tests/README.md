...
Run `npm test` for 40 synthetic tests: 36 storage/API tests and 4 rendered React DOM regressions. `npm run test:web` runs the latter separately. `npm run check` includes TypeScript and TSX test files. Tests use in-memory or temporary databases and never require private data, credentials, remote services, or `apps/web/dist`.
