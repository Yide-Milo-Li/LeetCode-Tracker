 ...
 * and completion state across Dashboard and Today views.
 */
 ...
 * the Dashboard summary card and the Today execution view.
 */
export function useDailyPlan(): UseDailyPlanReturn {
  ...
  const replaceOne = useCallback(async (item: PlanItem) => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : 'Failed to replace problem.');
    } finally {
  ...
  const replaceAllUnfinished = useCallback(async () => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : 'Failed to replace unfinished problems.');
    } finally {
  ...
  const onPracticeLogged = useCallback(async () => {
    changeRevision.current++;
    await refresh();
  }, [refresh]);
