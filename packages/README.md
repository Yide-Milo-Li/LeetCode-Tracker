...
- `database`: SQLite v9 storage, schema inspection, atomic imports/practices, durable replay fingerprints, revision conflicts, snapshots and offline recovery.
...
Schema creation and supported v3–v8 upgrades to v9 are embedded in the store. Old durations stay unknown (`null`); remarks are never interpreted as timing data. See the [data format](../docs/data-format.md) and [recovery guide](../scripts/README.md). Multi-provider AI integration lives on the server and is simulated by the default tests.
