...
try {
  ...
  await click('Change completion or time');
  ...
  await fill('[aria-label="Easy count"]', '2');
  await fill('[aria-label="Medium count"]', '0');
  await fill('[aria-label="Hard count"]', '0');
  await click('New Problems Only (No Review)', 'label');
